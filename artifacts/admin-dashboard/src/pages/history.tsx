            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {report.forbidden.map((ing) => (
                  <span
                    key={ing.name}
                    className="rounded-full border bg-red-50 px-2.5 py-0.5 text-xs text-red-800 border-red-200"
                  >
                    {ing.nameAr || ing.name}
                  </span>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {report.conditional.map((ing) => (
                  <span
                    key={ing.name}
                    className="rounded-full border bg-amber-50 px-2.5 py-0.5 text-xs text-amber-800 border-amber-200"
                  >
                    {ing.nameAr || ing.name}
                  </span>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {report.allowed.map((ing) => (
                  <span
                    key={ing.name}
                    className="rounded-full border bg-green-50 px-2.5 py-0.5 text-xs text-green-800 border-green-200"
                  >
                    {ing.nameAr || ing.name}
                  </span>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {report.unknown.map((ing) => (
                  <span
                    key={ing.name}
                    className="rounded-full border bg-gray-50 px-2.5 py-0.5 text-xs text-gray-700 border-gray-200"
                  >
                    {ing.nameAr || ing.name}
                  </span>
                ))}
...
export default function History() {
  ...
  const { data: items, isLoading } = useListAdminHistory({
    limit: PAGE_SIZE,
    offset,
  });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            All user food analyses across the platform
          </p>
        </div>
      </div>
                    ...
                    <tr key={item.id} className="border-b hover:bg-muted/20 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5 text-muted-foreground">
                          <TypeIcon type={item.analysisType} />
                          <span className="capitalize text-xs">{item.analysisType}</span>
                        </div>
                      </td>
